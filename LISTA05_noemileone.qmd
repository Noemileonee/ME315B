...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES * 1.20 AS NOVA_AREA FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES / 3.0 AS AREA_REDUZIDA FROM PRESERVE
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT FEE) AS QT_TAXAS FROM PRESERVE
WHERE FEE<> 0
```
...
```{sql connection=con}
SELECT COUNT(PNAME) AS TOTAL, COUNT(DISTINCT PNAME) AS NOMES_DISTINTOS FROM PRESERVE
...
SELECT SUM(SALARY) AS SOMA, 
       AVG(SALARY) AS MEDIA, 
       MAX(SALARY) AS MAXIMO_VALOR,
       MIN(SALARY) AS MINMO_VALOR 
       FROM EMPLOYEE
...
```{sql connection=con}
SELECT COUNT(*) AS TOTAL_FUNCIONARIOS FROM EMPLOYEE
WHERE DNO = "20"
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT DNO) AS TOTAL_DEPARTAMENTOS
FROM EMPLOYEE
```
